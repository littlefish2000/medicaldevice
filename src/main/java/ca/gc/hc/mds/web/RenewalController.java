...
public class RenewalController {
    ...
    public String findLicenceRenewsByLicenceIdForm(@RequestParam("authorizeId") BigDecimal licenceId, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
		uiModel.addAttribute("licenceId",licenceId);
		
		if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("licences", RenLicenceInfo.findRenLicenceInfosByLicenceNo(licenceId,firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) RenAuthCompany.countRenAuthCompanys() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("licences", RenLicenceInfo.findRenLicenceInfosByLicenceNo(licenceId));
        }
		return "renewal/renewbylicencelist";
    }	
